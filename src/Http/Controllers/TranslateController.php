...
class TranslateController
{
    public function index(Request $r)
    {
        ...
        foreach (['Translations','Reset','Search','Search key or text','Current Translations',
                  'Key','Actions','Save','Save All','All','missing','EN missing','AR missing','Saved successfully'] as $seed) {
            Trans::t($seed, $group);
        }
    ...
    public function update(Request $r, int $id)
    {
        ...
        $t = $line->text;
        if ($r->has('en')) $t['en'] = $data['en'] ?? '';
        if ($r->has('ar')) $t['ar'] = $data['ar'] ?? '';
        $line->update(['text'=>$t]);
    ...
    public function bulk(Request $r)
    {
        ...
        foreach ($ids as $id) {
            if (!$line = LanguageLine::find($id)) continue;
            $t = $line->text;
            if (array_key_exists($id, $data['values_en'] ?? [])) $t['en'] = $data['values_en'][$id] ?? '';
            if (array_key_exists($id, $data['values_ar'] ?? [])) $t['ar'] = $data['values_ar'][$id] ?? '';
            $line->update(['text'=>$t]);
        }

        cache()->flush();
        return back()->with('ok','bulk-saved');
    }
}
